import type { Webinix } from "../mod.ts";
import type { loadLib } from "./lib.ts";

export type Usize = number | bigint;

/**
 * Defines the signature for callback functions used with `Webinix.prototype.bind`.
 ...
 * @param event A `WebinixEvent` object containing details about the event.
 * @returns The result to be sent back to the UI (if the call originated from `webinix.call`), or `undefined`/`void` if no response is needed. Can be a direct value or a Promise.
 */
...
export interface WebinixEvent {
  /** The Webinix window instance associated with this event. */
  window: Webinix;
  /** The type of the event (e.g., Connected, Disconnected, MouseClick, Callback). See `Webinix.EventType`. */
  eventType: number;
