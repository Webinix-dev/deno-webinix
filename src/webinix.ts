...
export class Webinix {
   ...
   *
   * // Show the current time
   * await myWindow.show(`<html><script src="webinix.js">/script> <p>It is ${new Date().toLocaleTimeString()}</p> </html>`)
   *
   * // Show a local file
   ...
   *
   * // Show the current time
   * await myWindow.showBrowser(`<html><script src="webinix.js">/script> Hi, This is Chrome! </html>`, Webinix.Browser.Chrome)
   *
   * // Show a local file
  ...
  bind<T extends Datatypes | undefined | void>(
    ...
    const callbackResource = new Deno.UnsafeCallback(
      {
        // size_t webinix_interface_bind(..., void (*func)(size_t, size_t, char*, size_t, size_t))
      ...
      async (
        ...
        const args = {
          number: (index: number): number => {
            return Number(
              this.#lib.symbols.webinix_interface_get_int_at(
                BigInt(win),
                BigInt(event_number),
                BigInt(index),
              ),
          ...
          string: (index: number): string => {
            return (
              new Deno.UnsafePointerView(
                this.#lib.symbols.webinix_interface_get_string_at(
                  BigInt(win),
                  BigInt(event_number),
                  BigInt(index),
          ...
          boolean: (index: number): boolean => {
            return this.#lib.symbols.webinix_interface_get_bool_at(
              BigInt(win),
              BigInt(event_number),
              BigInt(index),
            ) as boolean;
  ...
  setFileHandler(callback: (url: URL) => Promise<string | Uint8Array>) {
    ...
    const callbackResource = new Deno.UnsafeCallback(
      ...
      async (
        ...
        // this may create a memory leak because Webinix cannot free
        // it, or cause memory corruption as Deno may free the buffer
        // before Webinix uses it. Therefore, the solution is to create
        // a safe Webinix buffer through Webinix API. This Webinix buffer will
        // be automatically freed by Webinix later.
        const webinix_buffer: Deno.PointerValue = _lib.symbols.webinix_malloc(
        ...
        if (typeof user_response === "string") {
          // copy `user_response` to `webinix_buffer` as String data
          ...
          // copy `user_response` to `webinix_buffer` as Uint8Array data
          const webinix_buffer_ref = new Uint8Array(
            Deno.UnsafePointerView.getArrayBuffer(
              webinix_buffer,
  ...
  getUrl(): string {
    return (
      new Deno.UnsafePointerView(
        this.#lib.symbols.webinix_get_url(
          BigInt(this.#window),
        ) as Deno.PointerObject<unknown>,
      ).getCString()
    ) as string;
  ...
  getChildProcessId(): number {
    return Number(
      this.#lib.symbols.webinix_get_child_process_id(BigInt(this.#window)),
    );
  }
  ...
  getBestBrowser(): number {
    return Number(
      this.#lib.symbols.webinix_get_best_browser(BigInt(this.#window)),
    );
  }
  ...
  setMinimumSize(width: number, height: number): void {
    this.#lib.symbols.webinix_set_minimum_size(
      BigInt(this.#window),
      width,
      height,
    );
   ...
   * @example
   * ```ts
   * const hc = Webinix.isHighContrast();
   * ```
   */
   ...
   *
   * @param {Webinix.Browser} browser - The browser to check
   * @return Returns True if the specified browser is available
   * @example
   * ```ts
   * const status = Webinix.browserExist(Webinix.Browser.Chrome);
   * ```
   */
   ...
   * @example
   * ```ts
   * Webinix.setDefaultRootFolder("/home/Foo/Bar/");
   * ```
   */
   ...
   * @example
   * ```ts
   * Webinix.openUrl("https://webinix.me");
   * ```
   */
   ...
   * @example
   * ```ts
   * const port = Webinix.getFreePort();
   * ```
   */
   ...
   * ```ts
   * const myWindow = new Webinix()
   * await myWindow.show(`<html><script src="webinix.js">/script> Your Page... </html>`)
   *
   * await Webinix.wait() // Async wait until all windows are closed
   *
   * // You can show windows again, or call Webinix.clean()
   * ```
   */
  static async wait() {
    Webinix.init();

    // Run Webinix main loop and render the WebView UI
    // _lib.symbols.webinix_wait();

    // TODO:
    // We should call `_lib.symbols.webinix_wait()` to render the WebView UI,
    // but this blocks the Deno main thread (`callbackResource`), which
    // prevents all Webinix events (clicks, calls, etc.) from being executed.
    //
    ...
    //
    // In the future, we should find a way to use `_lib.symbols.webinix_wait()`
    // to render the WebView UI without blocking Webinix events.
  ...
  static encode(str: string): string {
    ...
    return (
      new Deno.UnsafePointerView(
        _lib.symbols.webinix_encode(toCString(str)) as Deno.PointerObject<
          unknown
        >,
      ).getCString()
    ) as string;
  ...
  static decode(str: string): string {
    ...
    return (
      new Deno.UnsafePointerView(
        _lib.symbols.webinix_decode(toCString(str)) as Deno.PointerObject<
          unknown
        >,
      ).getCString()
    ) as string;
