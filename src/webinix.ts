...
export class Webinix {
  ...
  constructor() {
    this.#lib = loadLib();
    this.#lib.symbols.webinix_set_config(5, true); // Enable async calls
      ...
      async (
        ...
        const args = {
          number: (index: number): number => {
            return this.#lib.symbols.webinix_interface_get_int_at(win, event_number, index) as number
          },
          string: (index: number): string => {
            return (
              new Deno.UnsafePointerView(
                (this.#lib.symbols.webinix_interface_get_string_at(win, event_number, index) as Deno.PointerObject<unknown>)
              ).getCString()
            ) as string
          },
          boolean: (index: number): boolean => {
            return this.#lib.symbols.webinix_interface_get_bool_at(win, event_number, index) as boolean
          }
        }
