...
export class WebUi {
   ...
   * @example
   * ```ts
   * const webinix1 = new WebUi()
   * const webinix2 = new WebUi({ libPath: './local_webinix_2.dll' })
   * ```
   */
  constructor(
   ...
   * const webinix = new WebUi()
   * //Show the current time
   * webinix.showBrowser(`<html><p>It is ${new Date().toLocaleTimeString()}</p></html>`, WebUi.Browser.Firefox)
   * //Show a local file
    ...
    while (true) {
      ...
      if (
        Array.from(libs.values()).some((lib) =>
          !lib.symbols.webinix_interface_is_app_running()
        )
      ) break;
    }
  }
