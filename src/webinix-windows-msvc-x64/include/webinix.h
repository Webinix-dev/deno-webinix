...
WEBUI_EXPORT bool webinix_show(size_t window, const char* content);
...
WEBUI_EXPORT void webinix_close(size_t window);
// Close a specific window and free all memory resources.
WEBUI_EXPORT void webinix_destroy(size_t window);
// Close all open windows. `webinix_wait()` will break.
WEBUI_EXPORT void webinix_exit(void);
// Set the web-server root folder path for a specific window.
WEBUI_EXPORT bool webinix_set_root_folder(size_t window, const char* path);
// Set the web-server root folder path for all windows. Should be used before webinix_show().
...
WEBUI_EXPORT void webinix_set_timeout(size_t second);
...
WEBUI_EXPORT void webinix_run(size_t window, const char* script);
// Run JavaScript and get the response back (Make sure your local buffer can hold the response).
WEBUI_EXPORT bool webinix_script(size_t window, const char* script, size_t timeout, char* buffer, size_t buffer_length);
...
// Check if the app still running. This replaces `webinix_wait()`.
