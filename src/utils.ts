...
export function writeLib(
  ...
  const cachePath = path.join(getHome(), ".deno_webinix");
  const hash = (CRC32.buf(libBuffer) + 2 ** 32).toString(16);
  libName = `${hash}-${libName}`;
  fs.ensureDirSync(cachePath);
  const libPath = path.join(getHome(), ".deno_webinix", libName);
  if (!fs.existsSync(libPath) || clearCache) {
    Deno.writeFileSync(libPath, libBuffer);
  }
  if (!fs.existsSync(libPath)) {
    throw new WebinixError(`Can't write ${libName} at ${libPath}`);
  }
  return libPath;
}
...
export class WebinixError extends Error {}
